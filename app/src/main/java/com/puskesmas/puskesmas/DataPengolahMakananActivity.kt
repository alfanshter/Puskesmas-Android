...
import com.puskesmas.puskesmas.adapter.MakananAdapter
...
class DataPengolahMakananActivity : AppCompatActivity(),AnkoLogger {
    var desa: String? = null
    var api = ApiClient.instance()
    private val filePath = File(Environment.getExternalStorageDirectory().toString() +"/"+"TPM${System.currentTimeMillis()}"+".xls")
    lateinit var progressDialog: ProgressDialog
    private lateinit var mAdapter: MakananAdapter
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_data_pengolah_makanan)
                                ...
                                if (response.isSuccessful) {
                                    if (response.body()!!.data!!.isEmpty()){
                                        ...
                                        for (hasil in data!!.data!!) {
                                            notesList.add(hasil)
                                            mAdapter = MakananAdapter(notesList, this@DataPengolahMakananActivity)
                                            rvmakanan.adapter = mAdapter
                                            mAdapter.setDialog(object : MakananAdapter.Dialog{
                                                override fun onClick(
                                                    position: Int,
                                                    note: TpmModel
                                                ) {
                                                    startActivity<DetailLihatDataActivity>(
                                                        "tpm" to note.tpm,
                                                        "desa" to note.desa,
                                                ...
                                                override fun onHapus(position: Int, id: Int) {
                                                    val builder = AlertDialog.Builder(this@DataPengolahMakananActivity)
                                                    ...
                                                    builder.setPositiveButton(android.R.string.yes) { dialog, which ->
                                                        ...
                                                        api.deletetpm(id).enqueue(object : Callback<PostResponse>{
                                                            override fun onResponse(
    ...
    fun exportexcel(view : View){
        ...
        progressDialog.setCanceledOnTouchOutside(false)
        progressDialog.show()
        api.readtpmall().enqueue(object : Callback<TpmResponse>{
                ...
                try {
                    if (response.isSuccessful){
                        if (response.body()!!.data!!.isNotEmpty()){
                            ...
                            val cell2 = row7.createCell(1)
                            val cell3 = row7.createCell(2)
                            val cell4 = row7.createCell(3)
                            val cell5 = row7.createCell(4)
                            val cell6 = row7.createCell(5)
                            val cell7 = row7.createCell(6)
                            val cell8 = row7.createCell(7)
                            val cell9 = row7.createCell(8)
                            val cell10 = row7.createCell(9)
                            val cell11= row7.createCell(10)

                            val cell1row1 = row1.createCell(6)
                            cell1row1.setCellValue("REKAPITULASI TEMPAT PENGOLAHAN MAKANAN KECAMATAN SUKAMARA")

                            cell1.setCellValue("NAMA TPM")
                            cell2.setCellValue("GOLONGAN")
                            ...
                            cell11.setCellValue("EDIT TPM")
                            var row =7
                            for (data in response.body()!!.data!!){
                                row++
                                var datarow = hssfSheet.createRow(row)
                                val datacell = datarow.createCell(0)
                                val datacell2 = datarow.createCell(1)
                                val datacell3 = datarow.createCell(2)
                                val datacell4 = datarow.createCell(3)
                                val datacell5 = datarow.createCell(4)
                                val datacell6 = datarow.createCell(5)
                                val datacell7 = datarow.createCell(6)
                                val datacell8 = datarow.createCell(7)
                                val datacell9 = datarow.createCell(8)
                                val datacell10 = datarow.createCell(9)


                                datacell.setCellValue(data.tpm.toString())
                                datacell2.setCellValue(data.golongan.toString())
